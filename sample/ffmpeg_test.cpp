...
// 提取MPEG2: ffmpeg -i source.200kbps.768x320_10s.flv -vcodec mpeg2video -an -f mpeg2video
// source.200kbps.768x320_10s.mpeg2 播放：ffplay -pixel_format yuv420p -video_size 768x320 -framerate 25
...
int main(int argc, char **argv)
{
    ...
    if (strstr(filename, "264") != NULL) { video_codec_id = AV_CODEC_ID_H264; }
    ...
    // 查找解码器
    codec = avcodec_find_decoder(AV_CODEC_ID_H264); // AV_CODEC_ID_H264
    if (!codec) {
    ...
    while (data_size > 0) {
        ...
        ret = av_parser_parse2(parser, codec_ctx, &pkt->data, &pkt->size, data, data_size, AV_NOPTS_VALUE,
                               AV_NOPTS_VALUE, 0);
        if (ret < 0) {
